...
class App extends Component {
  ...
  getMoviesSpecs = () => {
    ...
    const api = `https://api.themoviedb.org/3/discover/movie?api_key=${API_KEY}&language=en-US&sort_by=popularity.desc&include_adult=false&include_video=false&page=${
    ...
    fetch(api)
      ...
      .then(movies => {
        this.setState({
          ...
          filterYear: {
            min: Math.min(
              ...movies.map(movie => movie.release_date.substring(0, 4))
            ),
            max: Math.max(
              ...movies.map(movie => movie.release_date.substring(0, 4))
            )
  ...
  render() {
    ...
    const movies = this.state.results.filter(
      x =>
        x.title.toLowerCase().includes(filterText.toLowerCase()) &&
        x.release_date.substring(0, 4) <= filterYear.max &&
        x.release_date.substring(0, 4) >= filterYear.min &&
        x.vote_average <= filterRating.max &&
        x.vote_average >= filterRating.min
    );
      ...
      <div>
        ...
        <div className="container-fluid row">
          <div className="col-md-3">
            ...
            <RangeFilterSection
              onFilterYear={value => this.handleYearChange(value)}
              onFilterRating={value => this.handleRatingChange(value)}
              orYear={orYear}
